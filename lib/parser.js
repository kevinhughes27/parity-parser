...
var parser = function(events) {
  ...
  var switchingDirectionEvents = groupedEvents.switchingDirectionEvents;
  var endOfDirectionEvents = groupedEvents.endOfDirectionEvents;
  var passingEvents = groupedEvents.passingEvents;
  var onFieldEvents = groupedEvents.onFieldEvents;
...
function processPassingEvents(events, stats) {
  for ( var i in events ) {
    for ( var j = 0; j < events[0].length; j += 2) {
      switch (events[i][j]) {
        case "Pass":
          if (events[i][j+2] == "" || events[i][j+2] === undefined) {
...
function groupEvents(events) {
  ...
  events.forEach(function(e) {
    ...
    if (isSwitchingDirectionEvent(token)) {
      switchingDirectionEvents.push(e);
    } else if (isDirectionEvent(token) && isEndOfDirectionEvent(e[2])) {
      endOfDirectionEvents.push(e.slice(2, 4));
      passingEvents.push(e.slice(4, e.length));
    } else if (isDirectionEvent(token)) {
      passingEvents.push(e.slice(2, e.length));
    } else if (isOnFieldEvent(token)) {
...
function isDirectionEvent(token) {
